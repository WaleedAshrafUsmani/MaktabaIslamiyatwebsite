...
class MenuBuilderFrontendRender
{
    ...
    public function render_frrontend_panel_menu($id,$type= null): string
    {
        $output = '';
        if (empty($id)){
            return $output;
    ...
    private function render_menu_item($menu_item, int $page_id, $default_lang, $type = null)
    {
        ...
        if ($ptype === 'custom'){
            ...
            if($type == 'category_menu'){
                $attributes_string['class'][] = 'cate-list';
            }

            $output .=  $this->render_li_start($pname,$attributes_string,$default_lang);
            $pname = SanitizeInput::esc_html($pname);
            $title = $pname;
            ...
            if(!empty($model)){
                $title = SanitizeInput::esc_html($model->name);
            ...
            $menu_item->pslug = SanitizeInput::esc_html($menu_item->pslug);
            ...
            if (!empty($matches[0])){
                $li_attributes = ['class' => 'cate-list menu-item-has-children current-menu-item'];
                $class_name = '\\'.$ptype;
                ...
                if ($instance->enable()){
                    $category = Category::find($menu_item->items_id ?? 0);
                     if(!is_null($category)){
                        $title = htmlspecialchars(strip_tags($category->name));
                        $output .=  $this->render_li_start($title,$li_attributes,$default_lang);
                        // get anchor data
                        $output .= $this->get_anchor_markup($title,[
                            'href' => '#' ,
                        ...
                        $output .= $instance->render($category ?? '',$default_lang,$menu_item->sub_cat_items_id ?? '',$title);
                    }
                }
            }else {
