 ...
 * */
function import_campaign($db, $adminId = 1): bool
{
    if (file_exists("campaign-database.sql")){
        $query = file_get_contents("campaign-database.sql");
        $query = str_replace("{admin_id}",$adminId,$query);
...
/* system info update */
function systemInstall($db_details): bool
{
    $status = false;
    ...
    if (file_exists('env-sample.txt')) {
        ...
        if (file_put_contents('../core/.env', $str)) {
            $status = true;
        }
    }
...
function setAdminDetails($db,$admin_details): bool
{
   ...
   $last_insert_id = $db->lastInsertId();
    ...
    import_campaign($db, $last_insert_id);
...
/* import database */
function importDatabase($db): bool
{
